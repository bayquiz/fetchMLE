...
def qs():
    ...
    if request.args:
        ...
        #print(params[0])
        answers = run(params[0], params[1])
        res2 = make_response(jsonify(answers), 200)
...
def run(dimensions, corners):
        ...
        #print("dimensions: ", dimensions, 'first: ', dimensions[0])
        #print("corners: ", corners, "first: ", corners[0])
        ...
        idk = [[a_, b_] for a_, b_ in zip(xx.flatten(), yy.flatten())]
        
        #now get the proper shape
        done = [idk[i:i + dimensions[0]] for i in range(0, len(idk), dimensions[0])]
        #print(done)
        return done
